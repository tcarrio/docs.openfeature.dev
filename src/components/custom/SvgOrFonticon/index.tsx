  ...
  override render() {
    ...
    return (
      <span style={{ marginBottom: 16 }}>
        {props.svg ? (
          <div style={{ display: 'inline', verticalAlign: 'middle' }} className={clsx(styles.svgIcon)}>
          <SvgComponent style={{ display: 'inline', width: props.size, height: props.size}}/>
        </div>          
        ) : (
          <FontAwesomeIcon style={{verticalAlign: 'sub', margin: 0, height: props.size, width: props.size }} className={clsx(styles.svg)} icon={iconDef} />
        )}
      </span>
    );
  }
